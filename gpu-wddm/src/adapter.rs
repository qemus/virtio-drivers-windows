    ...
    pub fn query_info(&self, query_info: &DXGKARG_QUERYADAPTERINFO) -> Result<(), NtStatus> {
        ...
        match query_info.Type {
            ...
            DXGK_QUERYADAPTERINFOTYPE::DXGKQAITYPE_DRIVERCAPS => {
                ...
                /*
                 * VirtIO-GPU has no mechanism to cancel a command once it has
                 * been placed on the control virtqueue. The current driver can
                 * therefore drain outstanding work, but it cannot prevent
                 * already queued DMA buffers from running as required for
                 * DMA-buffer-boundary preemption. Report the actual capability
                 * instead of asking VidSch to use a preemption/resubmission path
                 * that the driver cannot implement correctly.
                 */
